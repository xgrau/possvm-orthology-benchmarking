...
my @leaves = grep {$_->is_Leaf && defined $_->id && $_->id ne 'R'} $mytree->get_root_node->get_all_Descendents;
my @len;
my @tips;
for my $node (@leaves){
   my @path;
...
@leaves=grep {$_->is_Leaf && defined $_->id && $_->id ne 'R'} $cluster_node->get_all_Descendents;
if(@leaves==0){
 return \@cluster;
}
...
if (defined($myleaf))
{
 ...
 while($node_id!=$stop_point){
   @leaves=grep {$_->is_Leaf && defined $_->id && $_->id ne 'R'} $node->get_all_Descendents;
...
# we should check if the ancestor of the node1 (not the node1 itself) contains stoppers, because stoppers can be contained together 
# with the leaves
my @stopeers=();
   @stoppers=grep {$_->is_Leaf && $_->id == 'R'} $node2->get_all_Descendents;
   if ((scalar @stoppers >0)&&($node2->internal_id!=$root_id)){
    print LOG "Stopper (removed cluster) found. Node 1 is a cluster. Leave the cycle \n";   
    return 1;  
   } 
...
# if so, we just print them as a completed family and exit
  ...
  if (@leaves == $nu_of_species){
     ...
     print_cluster(\@cluster,\@family,\@empty,\@empty); 
...
#
 if($true){
  ...
  # check if tree still has leaves
   @des=grep {$_->is_Leaf && defined $_->id && $_->id ne 'R'} $tree->get_root_node->get_all_Descendents;
  # no leaves left: 
    if(@des==0){
