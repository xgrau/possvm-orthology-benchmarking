...
def clusters_mcl(evs, node_list, cluster_label="cluster", label_if_no_annot="NA"):
	...
	logging.info("Find communities greedy")
	clu_c = community.greedy_modularity_communities(evs_n)
	clu_c = { frozenset(c) for c in clu_c }
	logging.info("Find communities greedy num clusters = %i" % len(clu_c))
	...
	logging.info("Find communities greedy | num clustered genes = %i" % len(clu))
