...
for (ort_set in ort_sets) {
  ...
  pdf(sprintf("results_evaluation/eval_o2m_%s_classification_alluvial.pdf",id),height = 8, width = 7)
  for (fam in fam_list) {
  ...
  pdf(sprintf("results_evaluation/eval_o2m_%s_summary.pdf",id),height = 5, width = 7)
  ...
  # save table
  write.table(dia, file=sprintf("results_evaluation/eval_o2m_%s_summary.csv",id), quote = F, sep="\t",row.names = F)
