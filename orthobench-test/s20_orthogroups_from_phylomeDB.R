...
# load dictionary
dic = read.table(
  sprintf("results_phylomedb/dictionary_diamond_phylome_filt.csv", fam),
  stringsAsFactors = FALSE, col.names = c("phy_id", "new_id")
)
dic$species = stringr::str_split(dic$new_id, pattern="_", simplify = T)[,1]
sps_list = unique(dic$species)
sps_list = sps_list [ sps_list %in% ref$species ]
...
for (fam in fam_list) {
  ...
  ori_fn = sprintf("results_phylomedb/%s.diamond_phylome.human_orthologs.csv", fam)
  ori = read.table(
    ori_fn,
    sep = "\t", stringsAsFactors = FALSE,
    col.names = c("gene_i", "gene_j", "relationships", "CS", "trees", "synonyms_i"))
  
  if (nrow(ori)) {

    # load seed sequences (human)
    reo_fn = sprintf("results_phylomedb/%s.diamond_phylome.human.txt", fam)
    seed_gene_phy = read.table(reo_fn, stringsAsFactors = FALSE)[,1]
    seed_gene_ids = dic [ dic$phy_id %in% seed_gene_phy, "new_id" ]
  
    # expand synonyms
    ori$synonyms_i = paste(ori$gene_i, ori$synonyms_i, sep = " ")
    ori = ori[,c("synonyms_i", "gene_j")]
    # expand
    list_syn_i = stringr::str_split(ori$synonyms_i, pattern = " ")
    list_syn_i = sapply(list_syn_i, FUN = function(x) unique(x) )
    names(list_syn_i) = ori$gene_j
    orl = stack(list_syn_i)
    colnames(orl) = c("gene_i", "gene_j")
    orl = orl [ orl$gene_i != "" , ]
    
    # change to new ids
    orl = merge(orl, dic, by.x = "gene_i", by.y = "phy_id", all.x = TRUE, all.y = FALSE)
    orl = merge(orl, dic, by.x = "gene_j", by.y = "phy_id", all.x = TRUE, all.y = FALSE)
    
    # get members
    rog_members = unique(c(orl [ orl$new_id.x %in% seed_gene_ids , ]$new_id.x, orl [ orl$new_id.x %in% seed_gene_ids , ]$new_id.y))
    rog_members = rog_members [ !is.na(rog_members) ]
    
    non_members = unique(c(orl$new_id.x, orl$new_id.y))
    non_members = non_members [ !is.na(non_members) ]
    
    # output og in context
    out = data.frame(gene = c(rog_members, non_members), orthogroup = "other", stringsAsFactors = FALSE)
    out [ out$gene %in% rog_members, "orthogroup" ] = fam
    write.table(out, sprintf("results_phylomedb/%s.orthogroups.csv", fam), quote = FALSE, row.names = FALSE, sep ="\t")
...
for (ort_set in ort_sets) {
  ...
  pdf(sprintf("results_evaluation/eval_phylome_o2o_%s_classification_alluvial.pdf",id),height = 8, width = 7)
  for (fam in fam_list) {
    
    ort_fn = sprintf("results_phylomedb/%s.orthogroups.csv", fam)
    print(fam)
    
    if( file.exists(ort_fn)) {
      # read in possvm classification
      ort = read.table(ort_fn, sep="\t", header = T, stringsAsFactors = F, col.names = c("gene","orthogroup"))
  ...
  pdf(sprintf("results_evaluation/eval_phylome_o2o_%s_summary.pdf",id),height = 5, width = 7)
  ...
  # save table
  write.table(dia, file=sprintf("results_evaluation/eval_phylome_o2o_%s_summary.csv",id), quote = F, sep="\t",row.names = F)
