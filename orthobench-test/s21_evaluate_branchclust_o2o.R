...
for (ort_set in ort_sets) {
  ...
  pdf(sprintf("results_evaluation/eval_branchclust-%s_o2o_%s_classification_alluvial.pdf", many, id),height = 8, width = 7)
  for (fam in fam_list) {
  ...
  pdf(sprintf("results_evaluation/eval_branchclust-%s_o2o_%s_summary.pdf", many, id),height = 5, width = 7)
  ...
  # save table
  write.table(dia, file=sprintf("results_evaluation/eval_branchclust-%s_o2o_%s_summary.csv", many, id), quote = F, sep="\t",row.names = F)
