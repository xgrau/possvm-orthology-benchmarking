...
for (ort_set in ort_sets) {
  ...
  pdf(sprintf("results_evaluation/eval_root_o2o_ite_%s_classification_alluvial.pdf",id),height = 8, width = 7)
  for (fam in fam_list) {
  ...
  pdf(sprintf("results_evaluation/eval_root_o2o_ite_%s_summary.pdf",id),height = 5, width = 7)
  ...
  hist(dia$Fscore, breaks = 10, xlim = c(0,1),main="F-score", col="blue", border = "white", xlab = "F-score", cex.axis=0.9, cex.lab=0.9)
  title(sub=sprintf("n(>=0.95) = %i | mean = %.3f", sum(dia$Fscore >= 0.95), weighted.mean(dia$Fscore, dia$ref_size)))
  abline(v=0.95, lty=2, col="grey")
  
  hist(dia$precision, breaks = 10, xlim = c(0,1),main="Precision", col="blue", border = "white", xlab = "Precision", cex.axis=0.9, cex.lab=0.9)
  title(sub=sprintf("n(>=0.95) = %i | mean = %.3f", sum(dia$precision >= 0.95), weighted.mean(dia$precision, dia$ref_size)))
  abline(v=0.95, lty=2, col="grey")
  
  hist(dia$recall, breaks = 10, xlim = c(0,1),main="Recall", col="blue", border = "white", xlab = "Recall", cex.axis=0.9, cex.lab=0.9)
  ...
  # save table
  write.table(dia, file=sprintf("results_evaluation/eval_root_o2o_ite_%s_summary.csv",id), quote = F, sep="\t",row.names = F)
  
}

# store iter diagnostics
dia_ite = dia


# for midpoint rooted trees
for (ort_set in ort_sets) {
  ...
  pdf(sprintf("results_evaluation/eval_root_o2o_mid_%s_classification_alluvial.pdf",id),height = 8, width = 7)
  for (fam in fam_list) {
  ...
  pdf(sprintf("results_evaluation/eval_root_o2o_mid_%s_summary.pdf",id),height = 5, width = 7)
  layout(matrix(1:6, nrow = 2))
  plot(dia$precision, dia$recall, xlim = c(0,1), ylim=c(0,1), xlab = "Precision", ylab="Recall",
       col=alpha("blue", 0.6), main="Precision & recall", cex.axis=0.9, cex.lab=0.9)
  text(dia$precision, dia$recall, labels = dia$refOG, col=alpha("lightblue",0.8),cex=0.8)
  ...
  # save table
  write.table(dia, file=sprintf("results_evaluation/eval_root_o2o_mid_%s_summary.csv",id), quote = F, sep="\t",row.names = F)
