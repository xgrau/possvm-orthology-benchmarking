...
set_list = list(
  ...
  list(id = "ortb_all_mid_spst",  ref = "refOGs.csv", ort_fn = "orthobench_trees/raw/RefOG.*.possom_spst.ortholog_groups.csv"),
  list(id = "ortb_all_phylo_h",  ref = "refOGs.csv", ort_fn = "orthobench_trees/raw/RefOG.*.ccsh.ortholog_groups_from_ccs.csv"),
  list(id = "ortb_all_phylo_d",  ref = "refOGs.csv", ort_fn = "orthobench_trees/raw/RefOG.*.ccsd.ortholog_groups_from_ccs.csv"),
  ...
  list(id = "ortb_all_brc_m70",  ref = "refOGs.csv", ort_fn = "results_branchclust/RefOG.*.bc_clusters_70.csv"),
  list(id = "ortb_all_brc_m80",  ref = "refOGs.csv", ort_fn = "results_branchclust/RefOG.*.bc_clusters_80.csv")
)
...
for (set in set_list) {
  ...
  id = set$id
  ort_fn = set$ort_fn
  ort_fo = dirname(ort_fn)
  ort_fi = basename(ort_fn)
  ort_file_list = list.files(path = ort_fo , pattern = ort_fi)
  message(id)
  
  # load reference data
  ref = read.table(ref_fn, sep="\t", header = F, col.names = c("refOG", "gene"), stringsAsFactors=F)
  ref = ref[,c("gene","refOG")]
  ...
  for (ort_fn_i in ort_file_list) {
    
    # read in possvm classification
    fam = gsub("\\..*","", ort_fn_i)
    ort = read.table(sprintf("%s/%s", ort_fo, ort_fn_i), sep="\t", header = TRUE, stringsAsFactors = FALSE)
    colnames(ort)[1:2] = c("gene","orthogroup")
    
    if (nrow(ort)>0) {
      
      # subset ort to classifiable species
      ...
      if (nrow(ort)>0) {
        ...
        # merge
        ort = merge(ort[,c("gene","orthogroup")], rei, by.x = "gene", by.y = "gene", all.x = TRUE, all.y = FALSE)
        ...
        rand_index = fossil::adj.rand.index(clu_r, clu_q)
...
pdf(sprintf("%s/all_methods.pdf", out_fo), width = 6, height = 4.5)
