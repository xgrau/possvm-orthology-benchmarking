...
# list of datasets
set_list = list(
  list(id = "antp_all_ite_mcl",  ref = "results_searches/ANTP.seed.diamond-edit.csv", ort_fo = "results_trees/", prefix = "ANTP.possom",      sps_list = c("Drer","Galgal","Hsap","Mmus","Xentro","Dmel","Apimel","Tcas","Bflo")),
  list(id = "antp_ver_ite_mcl",  ref = "results_searches/ANTP.seed.diamond-edit.csv", ort_fo = "results_trees/", prefix = "ANTP.possom",      sps_list = c("Drer","Galgal","Hsap","Mmus","Xentro")),
  list(id = "antp_ins_ite_mcl",  ref = "results_searches/ANTP.seed.diamond-edit.csv", ort_fo = "results_trees/", prefix = "ANTP.possom",      sps_list = c("Dmel","Apimel","Tcas")),
  list(id = "antp_all_mid_mcl",  ref = "results_searches/ANTP.seed.diamond-edit.csv", ort_fo = "results_trees/", prefix = "ANTP.possom_mid",  sps_list = c("Drer","Galgal","Hsap","Mmus","Xentro","Dmel","Apimel","Tcas","Bflo")),
  list(id = "antp_all_ite_lpa",  ref = "results_searches/ANTP.seed.diamond-edit.csv", ort_fo = "results_trees/", prefix = "ANTP.possom_lpa",  sps_list = c("Drer","Galgal","Hsap","Mmus","Xentro","Dmel","Apimel","Tcas","Bflo")),
  list(id = "antp_all_ite_lou",  ref = "results_searches/ANTP.seed.diamond-edit.csv", ort_fo = "results_trees/", prefix = "ANTP.possom_lou",  sps_list = c("Drer","Galgal","Hsap","Mmus","Xentro","Dmel","Apimel","Tcas","Bflo")),
  list(id = "antp_all_ite_mclw", ref = "results_searches/ANTP.seed.diamond-edit.csv", ort_fo = "results_trees/", prefix = "ANTP.possom_mclw", sps_list = c("Drer","Galgal","Hsap","Mmus","Xentro","Dmel","Apimel","Tcas","Bflo")),
  list(id = "prds_all_ite_mcl",  ref = "results_searches/PRD.seed.diamond.csv",  ort_fo = "results_trees/", prefix = "PRD.possom",       sps_list = c("Drer","Galgal","Hsap","Mmus","Xentro","Dmel","Apimel","Tcas","Bflo")),
  list(id = "tale_all_ite_mcl",  ref = "results_searches/TALE.seed.diamond.csv", ort_fo = "results_trees/", prefix = "TALE.possom",      sps_list = c("Drer","Galgal","Hsap","Mmus","Xentro","Dmel","Apimel","Tcas","Bflo"))
...
for (set in set_list) {
  ...
  for (fam in fam_list) {
    ...
    ort_fn = sprintf("%s/%s.ortholog_groups.csv",ort_fo, prefix)
    print(paste(id,fam))
    ...
    # ref_pairs = apply(ref_pairs, 1, function(x) paste(sort(x), collapse = ","))
    # # get ordered pairs from query
    # ogi_genes = ort$gene [ ort$orthogroup_bool ]
    # ogi_pairs = t(combn(ogi_genes, 2))
    # ogi_pairs = apply(ogi_pairs, 1, function(x) paste(sort(x), collapse = ","))
    # # count
    ...
    dia = rbind(
      ...
      data.frame(
        ...
        FP=ev_FP,
        FN=ev_FN,
        TPw=ev_TP / (ort_hits_count-1),
        TNw=ev_TN / (ort_hits_count-1),
        FPw=ev_FP / (ort_hits_count-1),
        FNw=ev_FN / (ort_hits_count-1),
        TP_genes=ge_TP,
        FP_genes=ge_FP,
        FN_genes=ge_FN
      ))
  ...
  # Emms & Kelly-style accuracy stats 
  ek_precision = sum(dia$TPw) / ( sum(dia$TPw) + sum(dia$FPw) )
  ek_recall = sum(dia$TPw) / ( sum(dia$TPw) + sum(dia$FNw) )
  ek_Fscore = (2*ek_precision*ek_recall) / sum(ek_recall, ek_precision)
  ...
  title(sub=sprintf("n(>=.95) = %i | av = %.3f (EK = %.3f)", sum(dia$Fscore >= 0.95), weighted.mean(dia$Fscore, dia$ref_size), ek_Fscore))
  ...
  title(sub=sprintf("n(>=.95) = %i | av = %.3f (EK = %.3f)", sum(dia$precision >= 0.95), weighted.mean(dia$precision, dia$ref_size), ek_precision))
  abline(v=0.95, lty=2, col="grey")
  ...
  title(sub=sprintf("n(>=.95) = %i | av = %.3f (EK = %.3f)", sum(dia$recall >= 0.95), weighted.mean(dia$recall, dia$ref_size), ek_recall))
  abline(v=0.95, lty=2, col="grey")
