...
ort_sets = list(
  list(ort_fo="results_branchclust/", many=50),
  list(ort_fo="results_branchclust/", many=60),
  list(ort_fo="results_branchclust/", many=70),
  list(ort_fo="results_branchclust/", many=80)
)
...
for (ort_set in ort_sets) {
  ...
  for (spsi in names(sps_subsets)) {
    ...
    for (set in set_list) {
      ...
      pdf(sprintf("%s/eval_branchclust-%s_o2m_%s_%s_classification_alluvial.pdf",out_fo,many, id, spsi),height = 4, width = 6)
      for (fam in fam_list) {
        
        ort_fn = sprintf("%s/%s.bc_clusters_%s.csv",ort_fo, id, many)
        print(paste(id,fam))
      ...
      pdf(sprintf("%s/eval_branchclust-%s_o2m_%s_%s_summary.pdf",out_fo,many,id, spsi),height = 5, width = 7)
      ...
      # save table
      write.table(dia, file=sprintf("%s/eval_branchclust-%s_o2m_%s_%s_summary.csv",out_fo,many,id,spsi), quote = F, sep="\t",row.names = F)
