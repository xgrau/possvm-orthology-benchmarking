...
for (spsi in names(sps_subsets)) {
  ...
  for (set in set_list) {
    ...
    pdf(sprintf("%s/eval_o2m_mid_%s_%s_classification_alluvial.pdf",out_fo, id, spsi),height = 4, width = 6)
    for (fam in fam_list) {
    ...
    pdf(sprintf("%s/eval_o2m_mid_%s_%s_summary.pdf",out_fo,id, spsi),height = 5, width = 7)
    ...
    # save table
    write.table(dia, file=sprintf("%s/eval_o2m_mid_%s_%s_summary.csv",out_fo,id,spsi), quote = F, sep="\t",row.names = F)
