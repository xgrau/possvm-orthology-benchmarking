...
for (spsi in names(sps_subsets)) {
  ...
  for (set in set_list) {
    ...
    pdf(sprintf("%s/eval_o2o_lpa_%s_%s_classification_alluvial.pdf",out_fo, id, spsi),height = 4, width = 6)
    for (fam in fam_list) {
      
      ort_fn = sprintf("%s/%s.possom_lpa.ortholog_groups.csv",ort_fo, id)
      print(paste(id,fam))
    ...
    pdf(sprintf("%s/eval_o2o_lpa_%s_%s_summary.pdf",out_fo,id, spsi),height = 5, width = 7)
    ...
    # save table
    write.table(dia, file=sprintf("%s/eval_o2o_lpa_%s_%s_summary.csv",out_fo,id,spsi), quote = F, sep="\t",row.names = F)
