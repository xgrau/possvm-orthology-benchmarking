...
# input
ort_fo = "results_trees/permutations_ref/"
...
for (set in set_list) {
  ...
  for (per in perm_list) {
    ...
    # data
    per_fra = stringr::str_split(basename(per), pattern = "_|-", simplify = T)[,c(3)]
    per_rou = gsub("\\..*", "", stringr::str_split(basename(per), pattern = "_|-", simplify = T)[,c(4)])
...
pdf(sprintf("%s/eval_%s_permutations_ref.pdf",out_fo,id),height = 5, width = 7)
layout(matrix(1:6, nrow = 2))

# recall
boxplot(dia_sum$recall ~ dia_sum$perm_fraction, xlab = "Fraction of mis-placed genes", ylab = "Weighted recall", main="Recall", col="lightgray",
        ylim=c(0.2,1), cex.axis=0.7)
abline(h=0.937, lty=2, col="red")

# precision
boxplot(dia_sum$precision ~ dia_sum$perm_fraction, xlab = "Fraction of mis-placed genes", ylab = "Weighted precision", main="Precision", col="lightgray",
        ylim=c(0.2,1), cex.axis=0.7)
abline(h=0.976, lty=2, col="red")

# fscore
boxplot(dia_sum$Fscore ~ dia_sum$perm_fraction, xlab = "Fraction of mis-placed genes", ylab = "Weighted F1", main="F1 score", col="lightgray",
        ylim=c(0.2,1), cex.axis=0.7)
abline(h=0.942, lty=2, col="red")
dev.off()
